...
private FrameLayout initializeCroutonViewGroup(Resources resources) {
    ...
    params.setMargins(0, getKitKatTranslucentSize(), 0, 0);
    croutonView.setLayoutParams(params);
  ...
  @SuppressLint("NewApi")
private int getKitKatTranslucentSize() {
	  int topMargin = 0;
		
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
	    	TypedArray a = getActivity().getTheme().obtainStyledAttributes(
	    			new int[] {android.R.attr.windowTranslucentStatus, android.R.attr.actionBarSize});
	    	...
	    	if (a.getBoolean(0, false)) {
	    		...
	    		statusBarHeight = contentViewTop - statusBarHeight;
	    		
	    		if (statusBarHeight == 0) {
	    			statusBarHeight = (int) (24 * getActivity().getResources().getDisplayMetrics().density);
	    		}
	    		
	    		if (activity.getActionBar() != null && activity.getActionBar().isShowing()) {
	    			topMargin = a.getDimensionPixelOffset(1, 0) + statusBarHeight;
		    	} else {
